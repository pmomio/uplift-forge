...
import { Briefcase, Users, User, ClipboardList, ChevronRight, ChevronLeft, Check, Rocket } from 'lucide-react';
...
const PERSONA_OPTIONS: { value: Persona; label: string; description: string; icon: React.ReactNode; color: string }[] = [
  {
    ...
    description: 'Fish-eye view across all engineering work. Cross-project summaries, strategic KPIs, and high-level team performance.',
  ...
  },
  {
    ...
    description: 'Deep team insights across 1-10 projects. Individual engineer performance, continuous improvement, and full metric access.',
    icon: <Users size={24} />,
    color: 'cyan',
  },
  {
    ...
    description: 'Your own performance tracking front and center. Compare against team averages and track personal growth over time.',
    icon: <User size={24} />,
    color: 'emerald',
  },
  {
    ...
    description: 'Epic-level tracking with progress monitoring. Risk identification, cycle time analysis, and delivery forecasting.',
...
const OnboardingWizard: React.FC<OnboardingWizardProps> = ({ onComplete }) => {
  ...
  const handleNext = useCallback(async () => {
    if (step === totalSteps - 1) {
      ...
      try {
        ...
        if (!hasExistingProject && projectKey.trim()) {
    ...
    setStep(s => s + 1);
  }, [step, totalSteps, persona, projectKey, hasExistingProject, onComplete]);
  ...
  const canProceed = () => {
    ...
    if (step === projectStepIndex) return projectKey.trim().length > 0;
    ...
    if (step === personaStepIndex) {
      return (
        <div className="animate-slide-up">
          ...
          <p className="text-sm text-slate-400 text-center mb-8">This tailors your dashboard, metrics, and AI suggestions to your needs.</p>
      ...
      <div className="text-center animate-slide-up">
        ...
        <p className="text-sm text-slate-400 max-w-md mx-auto mb-6">
          ...
          {hasExistingProject
            ...
            : projectKey ? ` Project ${projectKey} is ready to sync.` : ''}
        </p>
        <p className="text-xs text-slate-500 max-w-sm mx-auto">
          You can change your role and customize metrics anytime in Settings.
        </p>
      </div>
    );
  };
